...
export default function TraditionalFoodPage() {
  ...
  const {
    ...
    isError,
    error,
  } = useDefaultQuery(['traditionalfood', productName, categories, page], url);
  ...
  /** 카테고리 필터*/
  function onChangeSetCategory(
    e: ChangeEvent<HTMLInputElement>,
    key: 'main' | 'sub' | 'detail' | 'foodType',
  ) {
  ...
  return (
    <section className={`${styles.traditional_food_page_container}`}>
      <Helmet>
        ...
        <meta
          name="description"
          content="과거 부터 현재까지 이어져 오는 민족의 혼이 담긴 전통음식을 찾아볼 수 있는 페이지 입니다. AI 가이드의 해설을 들어보세요."
        />
      </Helmet>
      ...
      <div className={styles.traditional_food_page_inner_boundary}>
        <GuideMessage
          ...
          totalCount={0}
        />
        <TraditionalFoodSearchForm
          action={searchAction}
          onSearch={onSearch}
          onReset={onReset}
        />
        <TraditionalFoodFilter
          onChange={onChangeSetCategory}
        ...
        <ListContainer
          container={'section'}
          className={`${styles.traditional_food_list_container}`}
        >
          <h2 className={styles.traditional_food_list_title}>
            전통음식 목록({totalCount})
          </h2>
          <span className={styles.traditional_food_category}>
            {(categories.main || '대분류') +
              '>' +
              (categories.sub || '중분류') +
              '>' +
              (categories.detail || '소분류')}
          </span>
          {isFetching ? (
            <p className={styles.traditional_food_list_loading_message}>
              데이터를 조회중입니다.
            </p>
          ) : hasProducts ? (
