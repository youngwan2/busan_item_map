...
export default function TraditionalFoodFilter({
  onChange,
  categories,
}: PropsType) {
  return (
    <form
      className={`${styles.traditional_food_filter_container}`}
      onSubmit={onSubmit}
    >
        ...
        {tarditionalFoodCategories.main.map((main) => {
          return (
            <div className={styles.input_container} key={main}>
              <input
                onChange={(e) => onChange(e, 'main')}
                name="main"
                id={'main-' + main}
                type="radio"
                defaultValue={categories.main}
                checked={categories.main === main}
                data-value={main}
              />
          ...
          return (
            <div className={styles.input_container} key={sub}>
              <input
                ...
                id={'sub-' + sub}
                type="radio"
                defaultValue={categories.sub}
                checked={categories.sub === sub}
                data-value={sub}
              />
          ...
          return (
            <div className={styles.input_container} key={detail}>
              <input
                ...
                type="radio"
                key={detail}
                defaultValue={categories.detail}
                checked={categories.detail === detail}
                data-value={detail}
              />
        ...
        {tarditionalFoodCategories.foodType.map((foodType) => {
          return (
            <div className={styles.input_container} key={foodType}>
              <input
                onChange={(e) => onChange(e, 'foodType')}
                name="foodtype"
                id={'foodtype-' + foodType}
                type="radio"
                key={foodType}
                defaultValue={categories.foodType}
                checked={categories.foodType === foodType}
                data-value={foodType}
              />
