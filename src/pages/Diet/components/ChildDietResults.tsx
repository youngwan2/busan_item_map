      ...
      <>
        <section className={styles.diet_item_box_con} id="child-diet-results">
          ...
          {childDietList.map((diet, index) => {
            return (
              <ul
                key={diet.MEAL_NM || '알 수 없음'}
                className={styles.diet_item_box}
                onClick={() => {
                  setCloseModal((old) => (old = !old));
                  setChoiceItemIndex(index);
                }}
              >
                <li className={styles.diet_item_title}>
                  <h3>{diet.MEAL_NM}</h3>
                  <ul className={styles.diet_item_list_con}>
                    <li className={styles.diet_item_list}>
                      <strong>열량</strong> {diet.CALORIE_QY}kcal
                    </li>
                    <li className={styles.diet_item_list}>
                      <strong>단백질</strong> {diet.PROTEIN_QY}g
                    </li>
                    <li className={styles.diet_item_list}>
                      <strong>지방</strong> {diet.FAT_QY}g
                    </li>
                    <li className={styles.diet_item_list}>
                      <strong>탄수화물</strong> {diet.CARBOH_QY}g
                    </li>
                  </ul>
                </li>
              </ul>
        ...
        {closeModal ? (
          <ChildDietDetailModal
            setCloseModal={setCloseModal}
            dietList={childDietList}
            index={choiceItemIndex}
            setIndex={setChoiceItemIndex}
          />
        ) : null}
      </>
    );
