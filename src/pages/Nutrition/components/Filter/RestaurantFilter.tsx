...
export default function RestaurantFilter({
  onChangeRestaurantValue,
}: PropsType) {
  const [isAllRestaurant, setIsAllRestaurant] = useState(false);
  const visibleRestaurantCount = isAllRestaurant
    ? fullRestaurantCount
    : limitRestaurantCount;

  function handleRestaurantVisibility(
    state: boolean,
      ...
      <div className={styles.filter_contents}>
        {restaurants.slice(0, visibleRestaurantCount).map((restaurant) => {
          return (
            <div className={styles.input_container} key={restaurant.id}>
              <input
                onChange={onChangeRestaurantValue}
                name="restaurant"
                type="checkbox"
                id={restaurant.name + restaurant.id}
                value={restaurant.name}
              />
              <label htmlFor={restaurant.name + restaurant.id}>
                {restaurant.name}
              </label>
            </div>
          );
        ...
        {!isAllRestaurant ? (
          <button
            ...
            onClick={() =>
              handleRestaurantVisibility(isAllRestaurant, setIsAllRestaurant)
            }
          >{`...${hiddenRestaurantCount}개 더보기`}</button>
        ) : (
          <button
