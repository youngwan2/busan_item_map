...
export default function RecipeContents({ recipe }: PropsType) {
  return (
    <div
      key={recipe.RCP_SEQ}
      className={styles.recipe_contents_inner_container}
    >
      {/* 레시피 개요 */}
            ...
            <li className={styles.recipe_ingredient}>
              ...
              <p>
                {recipe.RCP_PARTS_DTLS.split(',').map((splitText, i) => {
                  return (
                    <span key={splitText} className={styles.split_container}>
                      <mark>{i + 1}</mark>{' '}
                      <span className={styles.split_text}>{splitText}</span>
                    </span>
                  );
                })}
              </p>
      ...
      <div className={styles.recipe_contents_detail}>
        ...
        <div className={styles.recipe_figure_container}>
          {[...Array(8)].map((_, index) => (
            ...
            >
              <img
                src={recipe[`MANUAL_IMG0${index + 1}`]}
                alt={`만드는법${index + 1}`}
              />
