...
export default function LocalFoodDetailPage() {
    ...
    const { id } = useParams()
    const { data: localFood, isError, isPending, error } = useDefaultQuery(['localfood', id], '/localfood/' + id) as { data: localFoodType, isError: boolean, isPending: boolean, error: Error | null }
    const { content, content_url, create_at, update_at, keyword, lcc_address, main_thumb_url, rel_rest_address, rel_rest_name, rel_rest_tel, sub_title, title } = localFood || { content: '', content_url: '', create_at: '', update_at: '', keyword: '', lcc_address: '', main_thumb_url: '', rel_rest_address: '', rel_rest_name: '', rel_rest_tel: '', sub_title: '', title: '' }
    ...
    return (
        <section className={styles.LocalFood_Detail}>
            ...
            <article className={styles.left_content}>
                ...
                <img className={styles.thumb} src={main_thumb_url || '/not-image.png'} alt="썸네일 이미지" />
            ...
            <article className={styles.right_content}>
                ...
                <div className={styles.rel_rest || '조회된 데이터가 없습니다.'}>
                    <hr />
                    <h3>연관식당</h3>
                    <span>{rel_rest_name}({rel_rest_address})</span>
