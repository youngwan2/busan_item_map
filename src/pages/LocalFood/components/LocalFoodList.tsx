            ...
            <ul className={styles.localfood_ul} id='localfood-ul'>
                {localfoods.map((localfood) => {
                    ...
                    <li className={styles.localfood_li} key={local_food_id}>
                        <img className={styles.main_thumb} src={main_thumb_url || '/not-image.png'} width={250} height={250}></img>
                        <p className={styles.main_thumb_title}>{title}</p>
                        <button onClick={()=>onClickPageChange(local_food_id)} aria-label='세부 페이지 이동 버튼'><span><HiSearchCircle/></span>더보기</button>
                    </li>
                    )
                })}
            </ul>
