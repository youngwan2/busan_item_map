...
export default function LocalCategoryGrid({ categoryName, onSetPrdkind }: PropsType) {
    return (
                    ...
                    <button
                        ...
                        {category.name !== ''
                            ? <div className={styles.img_boundary}>
                                <div aria-label='추천품목 이미지' className={`${styles.category_grid_cell_img}`} />

                            </div>
                            : null
                        }

                        <p aria-label='추천품목 이름'>{category.name !== '' ? category.name : '전체'}</p>
                    </button>
                ))}
            </div>
        </section>
