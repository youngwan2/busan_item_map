...
export interface MarkerType { position: { lat: number, lng: number }, content: string }
        ...
        <>
            <div className={styles.maps_container}>
                <Map
                    ...
                    {/* 로드뷰 활성 유무 따른 지도 렌더 */}
                    {isAtive
                        ? <MapLoadViewMarker marker={marker} pickCenter={pickCenter} name={name ?? ''} onPositionChange={handlePositionChange} />
                        :
                        // 로드뷰 비활성화 시 보여줄 마커
                        <MapMarker
