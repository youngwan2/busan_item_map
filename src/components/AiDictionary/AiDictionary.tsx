...
interface PropsType { searchValue: string }
export default function AiDictionary({ searchValue }: PropsType) {

    const [isOpen, setIsOpen] = useState(false)
    const [response, setResponse] = useState({ searchValue: '', aiResponse: '', recommand: [] })
    ...
    async function onSearch(value: string) {
        ...
        setResponse(old => ({ ...old, searchValue: value, aiResponse:parseResponse.response, recommand:parseResponse.recommand }))
    }
    ...
    return (
        ...
        {createPortal(
            <AiDictionaryModal response={response} isOpen={isOpen} onCloseModal={onModalDisplay} />,
            document.body
        )}
  </>
    )
